...
// Non-blocking variants used by the Owner Processing Workspace. Existing synchronous
// endpoints remain intact for compatibility; these endpoints return immediately and
// execute the exact same production service functions in the background.
router.post('/analyze/async', auditAdminAction('sso.application_analyzer.analyze_async','sso_application_analyzer'), (req,res,next) => {
  try {
    const body = req.body || {};
    const operation = ownerOperation.create({ type:'repository-analysis', label:'Repository analysis', run: async ctx => {
      ctx.progress('REPOSITORY', `Reading ${body.repository || 'target repository'}@${body.branch || 'default branch'}…`, 5);
      const result = await analyze(body, { onProgress: ctx.progress, isCancelled: ctx.isCancelled });
...
router.post('/plan/async', auditAdminAction('sso.application_analyzer.plan_async','sso_application_analyzer'), (req,res,next) => {
  try {
    const body = req.body || {};
    const operation = ownerOperation.create({ type:'source-review-plan', label:'Source review plan', run: async ctx => {
      ctx.progress('SOURCE REVIEW', `Preparing SHA-bound review plan for ${Array.isArray(body.files) ? body.files.length : 0} reviewed candidates…`, 10);
      const result = await plan(body);
      ctx.progress('PLAN READY', `Signed review plan created for ${result.reviewedFiles?.length || 0} files.`, 100);
      return result;
    }});
    res.status(202).json({ success:true, operation });
  } catch (e) { next(e); }
