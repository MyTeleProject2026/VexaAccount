...
router.post('/plan', auditAdminAction('sso.application_analyzer.plan','sso.application_analyzer'), async (req,res,next)=>{try{res.json(await plan(req.body||{}));}catch(e){next(e);}});
router.post('/analyze/async', auditAdminAction('sso.application_analyzer.analyze_async','sso.application_analyzer'), (req,res,next)=>{try{const payload={...(req.body||{})};const operation=ownerOperation.create({type:'repository-analysis',label:'Repository analysis',payload,run:async (_ignored,ctx)=>{ctx.progress('QUEUED','Analysis accepted; starting server-side worker.',1);await new Promise(resolve=>setTimeout(resolve,250));const result=await analyze(payload,ctx);ctx.progress('ANALYSIS COMPLETE',`Inspected ${result.summary?.sourceFilesInspected ?? result.files?.length ?? 0} source files and detected the target stack.`,100);return result;}});res.status(202).json({success:true,operation});}catch(e){next(e);}});
router.post('/plan/async', auditAdminAction('sso.application_analyzer.plan_async','sso.application_analyzer'), (req,res,next)=>{try{const operation=ownerOperation.create({type:'source-review-plan',label:'Source review plan',payload:req.body||{}});res.status(202).json({success:true,operation});}catch(e){next(e);}});
