...
router.patch('/messages/:id/star',async(req,res,next)=>{try{const starred=req.body?.starred===undefined?true:Boolean(req.body.starred);const [r]=await pool.query('UPDATE vexamail_messages SET starred=? WHERE id=? AND user_id=?',[starred?1:0,req.params.id,uid(req)]);if(!r.affectedRows)return res.status(404).json({success:false,message:'Message not found'});res.json({success:true,starred})}catch(e){next(e)}});router.delete('/messages/:id',async(req,res,next)=>{try{const [r]=await pool.query('UPDATE vexamail_messages SET is_trashed=1,deleted_at=NOW() WHERE id=? AND user_id=?',[req.params.id,uid(req)]);if(!r.affectedRows)return res.status(404).json({success:false,message:'Message not found'});res.json({success:true})}catch(e){next(e)}});router.post('/messages/:id/read',async(req,res,next)=>{try{const [r]=await pool.query('UPDATE vexamail_messages SET is_read=1 WHERE id=? AND user_id=?',[req.params.id,uid(req)]);if(!r.affectedRows)return res.status(404).json({success:false,message:'Message not found'});res.json({success:true})}catch(e){next(e)}});
