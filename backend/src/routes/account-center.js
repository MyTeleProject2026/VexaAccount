...
router.get('/sessions', async (req,res,next)=>{ try { const [rows]=await pool.query(`SELECT id, client_id, scope, created_at, last_seen_at, expires_at FROM sso_sessions WHERE user_id=? AND revoked_at IS NULL AND expires_at > NOW() ORDER BY last_seen_at DESC`, [req.accountUser.userId]); res.json({success:true,sessions:rows}); } catch(error){next(error);} });
