...
const { verify: verifyPlanToken } = require('../services/ssoIntegrationPlanToken.service');
...
router.get('/catalog', auditAdminAction('sso.application_kit.catalog','sso_application_kit'), (req,res) => res.json({
  ...
  security:['Authorization Code','S256 PKCE','encrypted stateless PKCE transaction','server-side client secret','application-owned JWT session','no third-party cookie/token copying','signed expiring source-review plan'],
  workflow:['read-only source analysis','cryptographically signed precise source plan','generated additive kit','explicit Owner review before replacement','explicit deployment action'],
...
router.post('/generate', auditAdminAction('sso.application_kit.generate','sso_application_kit'), (req,res,next) => {
  try {
    ...
    res.status(200).json({success:true,sourcePlan:{repository:token.repository,branch:token.branch,expiresAt:token.exp,reviewedFiles:token.reviewedFiles},...generated});
  } catch(e){next(e);}
});
