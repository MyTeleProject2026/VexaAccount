...
router.get('/applications', auditAdminAction('sso.registry.list', 'sso_application'), async (req, res, next) => {
  try {
    const [rows] = await withDeadline(pool.query({
      sql: `SELECT r.client_id,r.display_name,r.application_key,r.environment,r.status,
    ...
    return res.json({ success: true, applications: rows.map(row => ({
      ...row,
      ...
      profile: { applicationUrl: row.application_url || null, framework: row.framework || null, githubRepository: row.github_repository || null, githubBranch: row.github_branch || null, renderServiceId: row.render_service_id || null, cloudinaryFolder: row.cloudinary_folder || `vexaaccount/applications/${row.client_id}` },
    ...
    if (error?.code === 'VEXA_REQUEST_TIMEOUT' || error?.code === 'PROTOCOL_SEQUENCE_TIMEOUT' || /timeout/i.test(String(error?.message || ''))) return res.status(503).json({ success: false, message: 'SSO application registry query timed out' });
    return next(error);
  }
});
...
router.patch('/applications/:clientId', auditAdminAction('sso.registry.application.update', 'sso_application'), async (req, res, next) => {
  try {
    ...
    let normalizedRedirectUris = null;
    let normalizedAllowedScopes = null;

    if (hasRedirectUris) {
      if (!Array.isArray(redirectUris)) return res.status(400).json({ success:false, message:'redirectUris must be an array' });
      normalizedRedirectUris = [...new Set(redirectUris.map(v => String(v || '').trim()).filter(Boolean))];
      if (!normalizedRedirectUris.length) return res.status(400).json({ success:false, message:'At least one redirect URI is required' });
      for (const uri of normalizedRedirectUris) {
        try { const u = new URL(uri); if (u.protocol !== 'https:' || u.username || u.password || u.hash) throw new Error('unsafe redirect URI'); }
        catch (_) { return res.status(400).json({ success:false, message:`Invalid redirect URI: ${uri}` }); }
      }
    }
    if (hasAllowedScopes) {
      if (!Array.isArray(allowedScopes)) return res.status(400).json({ success:false, message:'allowedScopes must be an array' });
      const supported = new Set(['openid','profile','email','account','session','applications','notifications']);
      normalizedAllowedScopes = [...new Set(allowedScopes.map(v => String(v || '').trim()).filter(Boolean))];
      const unsupported = normalizedAllowedScopes.filter(scope => !supported.has(scope));
      if (unsupported.length) return res.status(400).json({ success:false, message:`Unsupported scope(s): ${unsupported.join(', ')}` });
      if (!normalizedAllowedScopes.includes('openid')) normalizedAllowedScopes.unshift('openid');
    }
    if (!updates.length && !hasRedirectUris && !hasAllowedScopes) return res.status(400).json({ success:false, message:'No supported application fields supplied' });
    ...
    try {
      ...
      if (updates.length) {
        ...
        if (!exists.length) { await connection.rollback(); return res.status(404).json({ success:false, message:'SSO application not found' }); }
      }
      if (hasRedirectUris || hasAllowedScopes) {
        const clientUpdates = [], clientValues = [];
        if (hasRedirectUris) { clientUpdates.push('redirect_uris=?'); clientValues.push(JSON.stringify(normalizedRedirectUris)); }
        if (hasAllowedScopes) { clientUpdates.push('allowed_scopes=?'); clientValues.push(JSON.stringify(normalizedAllowedScopes)); }
        ...
        if (!result.affectedRows) { await connection.rollback(); return res.status(404).json({ success:false, message:'SSO client record not found for application' }); }
      }
      await connection.commit();
    } catch (error) { try { await connection.rollback(); } catch (_) {} throw error; }
    finally { connection.release(); }

    return res.json({ success:true, message:'SSO application updated', clientId, ...(hasRedirectUris ? { redirectUris: normalizedRedirectUris } : {}), ...(hasAllowedScopes ? { allowedScopes: normalizedAllowedScopes } : {}) });
  } catch (e) { next(e); }
});
