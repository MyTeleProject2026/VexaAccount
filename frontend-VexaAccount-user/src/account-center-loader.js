...
const AUTH=/^#\/(login|signin|register|forgot-password|verify-email|reset-password|login-2fa)(?:[/?]|$)/i;const SWITCHER=/^#\/account-switcher(?:[/?]|$)/i;
...
async function load(force=false){if(loading||window.__VEXA_ACCOUNT_CENTER_READY__||AUTH.test(location.hash||'')||SWITCHER.test(location.hash||'')||isSso())return;loading=true;status('Loading Account Center…','Checking your secure VexaAccount session.');try{if(!(await ensureSession())){status('Sign in required','Your secure session was not found. Please sign in to continue.','error',true);return;}await loadScript('./src/account-center-toast-guard.js?v=20260907-01','account-center-toast-guard.js');await loadScript('./src/account-center-runtime-v2.js?v=20260925-05','account-center-runtime-v2.js');await loadScript('./src/account-center-premium-theme.js?v=20260907-01','account-center-premium-theme.js');const started=Date.now();while(Date.now()-started<10000){const r=root();if(r&&r.querySelector('#vx-content'))break;await new Promise(resolve=>setTimeout(resolve,50));}const r=root();if(!r||!r.querySelector('#vx-content'))throw new Error('Account Center did not finish initialization');window.__VEXA_ACCOUNT_CENTER_READY__=true;r.style.display='block';const a=app();if(a){a.style.display='none';a.replaceChildren()}}catch(e){console.error('[VexaAccount] Account Center startup failed',e);status('Unable to start Account Center',e?.message||'The secure account workspace could not be started.','error',true);}finally{loading=false}}
