...
let loading=false;
function load(){if(loading||window.__VEXA_ACCOUNT_CENTER_READY__||AUTH.test(location.hash||'')||!token())return;loading=true;const s=document.createElement('script');s.src='./src/account-center-runtime-v2.js?v=20260902-27';s.dataset.vexaAccountRuntime='account-center-runtime-v2.js';s.onload=()=>{loading=false};s.onerror=()=>{loading=false;console.error('[VexaAccount] Account Center runtime failed to load')};document.body.appendChild(s)}
