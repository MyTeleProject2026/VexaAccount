...
return d};
async function bootstrapCookieSession(){try{if(getToken())return;const hash=location.hash||'';if(/^#\/(sso\/authorize)/i.test(hash))return;const d=await window.vexaSessionFetch();if(d?.success===true&&d.user){if(/^#\/(login|signin)$/i.test(location.hash||'')){location.hash='#/';}window.dispatchEvent(new CustomEvent('vexa:auth-changed',{detail:{user:d.user,source:'cookie-session'}});window.dispatchEvent(new Event('vexa-auth-ready'));}}catch(error){console.debug('[VexaAccount] cookie session bootstrap skipped',error?.message||error)}}
