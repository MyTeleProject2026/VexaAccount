...
if(/\/api\//.test(url)){try{const d=await response.clone().json();const fresh=pickToken(d);if(fresh)saveToken(String(fresh));if(response.ok&&d?.success&&/\/api\/account\/change\/password\/verify$/.test(url))clearToken();}catch{}}
