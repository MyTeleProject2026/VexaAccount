(()=>{
  'use strict';
  if(window.__VEXA_OWNER_OPERATION_RUNTIME_V2__)return;
  window.__VEXA_OWNER_OPERATION_RUNTIME_V2__=true;
  ...
  function ensurePanel(){
    ...
    host.innerHTML=`<style>
      ...
      <header class="vo-head"><div><div class="vo-title">OWNER OS // LIVE EXECUTION</div><div class="vo-sub">Real server operation · live worker state · non-blocking workspace</div></div><div><button data-minimize>Minimize</button></div></header>
      <div class="vo-body">
    ...
    host.querySelector('[data-cancel]').onclick=async()=>{const id=host.dataset.operationId;if(!id)return;try{await fetch(`${API}/api/sso-application-analyzer/operations/${encodeURIComponent(id)}/cancel`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'}})}catch(e){renderError(e)}};
    return host;
  }

  function renderError(error){
    const p=ensurePanel(); const d=p.querySelector('[data-detail]'); d.className='vo-detail vo-error'; d.textContent=error?.message||String(error||'Operation failed');
  }

  function render(snapshot,event){
    const p=ensurePanel();
  ...
  function updateTimer(){
    const p=state.panel;if(!p)return;const id=p.dataset.operationId;if(!id)return;const w=state.watchers.get(id);if(!w?.snapshot)return;const start=Date.parse(w.snapshot.startedAt||w.snapshot.createdAt);const end=terminal(w.snapshot.status)&&w.snapshot.completedAt?Date.parse(w.snapshot.completedAt):Date.now();p.querySelector('[data-elapsed]').textContent=formatElapsed(Math.max(0,end-start));
  }
  ...
  function finish(id,snapshot){
    const w=state.watchers.get(id);if(!w)return;w.snapshot=snapshot;render(snapshot);updateTimer();if(w.poll){clearInterval(w.poll);w.poll=null}if(w.source){try{w.source.close()}catch(_){}w.source=null}if(terminal(snapshot.status)){state.watchers.delete(id);if(snapshot.status==='completed')w.onComplete?.(snapshot.result??snapshot);else w.onError?.(snapshot.error||new Error(snapshot.detail||`Operation ${snapshot.status}`));}
  }

  function attach(id,handlers={}){
    if(state.watchers.has(id))return state.watchers.get(id);
    const w={id,snapshot:null,onComplete:handlers.onComplete,onError:handlers.onError,poll:null,source:null,streamFailed:false};state.watchers.set(id,w);ensurePanel().dataset.operationId=id;
    const consume=snapshot=>{w.snapshot=snapshot;render(snapshot);updateTimer();if(terminal(snapshot.status))finish(id,snapshot)};
    getState(id).then(consume).catch(e=>{renderError(e);w.onError?.(e)});
    if(typeof EventSource==='function'){
      try{
        const source=new EventSource(`${API}/api/sso-application-analyzer/operations/${encodeURIComponent(id)}/stream`,{withCredentials:true});w.source=source;
        source.onmessage=e=>{try{const payload=JSON.parse(e.data);consume(payload.operation)}catch(_){}};
        source.addEventListener('state',e=>{try{const payload=JSON.parse(e.data);consume(payload.operation)}catch(_){}});
        source.addEventListener('event',e=>{try{const payload=JSON.parse(e.data);consume(payload.operation)}catch(_){}});
        source.addEventListener('heartbeat',e=>{try{const payload=JSON.parse(e.data);consume(payload.operation)}catch(_){}});
        source.onerror=()=>{if(terminal(w.snapshot?.status))return;w.streamFailed=true;if(w.poll)return;w.poll=setInterval(async()=>{try{const s=await getState(id);consume(s)}catch(e){renderError(e)}},3000)};
      }catch(_){w.streamFailed=true}
    }
    if(!w.poll)w.poll=setInterval(async()=>{if(terminal(w.snapshot?.status)){clearInterval(w.poll);w.poll=null;return}try{consume(await getState(id))}catch(e){renderError(e)}},7000);
    return w;
  }
  ...
  const api={
    ...
    cancel(id){return fetch(`${API}/api/sso-application-analyzer/operations/${encodeURIComponent(id)}/cancel`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'}})}
  };
