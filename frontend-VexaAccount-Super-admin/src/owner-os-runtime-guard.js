...
async function recover(){if(retries>=MAX_BOOT_RETRIES)return;if(!window.vexaOwnerOS?.reload)return;if(!(await sessionIsStillValid()))return;retries+=1;showRestoring();setTimeout(()=>{try{window.vexaOwnerOS.reload()}catch{}} ,Math.min(1500*retries,4500))}
const observer=new MutationObserver(()=>{if(document.querySelector('.os-login-card')&&window.vexaOwnerOS?.reload)recover()});
function start(){observer.observe(document.documentElement,{subtree:true,childList:true});setTimeout(()=>{if(document.querySelector('.os-login-card'))recover()},1800)}
